function [ss_estim] = iekfW2ndDeriv(y, t, is_artifact, m, num_iters,  num_particles, verbose)
%iekfW2ndDeriv estimates a multi-peak, state-space model for a spectrogram using
...
% initial reference trajectory. The difference from iekf is that it treats
% the filter update as a Newton-Raphson optimization of the posterior mode
% that utilizes the 2nd-derivate of the observation function.
%
% NOTE: Like iekf, this function explicitly overwrites the state transition by 
...
% OUTPUTS:
%   ss_etim -- estimate structure for StateSpaceMultiPeak estimate. 
%              Contains:
%                num_particles
%                num_iters
        ...
        if is_artifact(kk-1) % is_artifact was not augmented with zero in front, so is lagging.
            ...
            parfor jj = 1:m.numCombos
                ...
                if f_cast_net
                    ...
                    sse_draws = sum(resid_draws.^2./repmat(diag(R_parconst.Value),[1 num_particles]),1);
                    
                    %Take minimum SSE particle
                    [~,idx_min_sse] = min(sse_draws);
                    eta_next = x_draws(idx_min_sse,:)';
                    trajec_curr = yhat_draws(:,idx_min_sse);
                    sse_draw = (y_k-trajec_curr)'*(R_parconst.Value\(y_k-trajec_curr));
                else
                ...
                for ii = 1:iterations
                    ...
                    % Jacobian of approximate posterior
                    invRresid = R_parconst.Value\(y_k - yf_curr{jj});
                    Jacob = M_curr{jj}'*invRresid - Pp_curr \ (eta_curr-xp_curr);
                    
                    % Hessian of approximate posterior
                    Hxx_curr = m.getHxx(eta_curr,omega,curr_combo);
                    term = squeeze(sum(repmat(invRresid,[1 length(xp_curr) length(xp_curr)]).* Hxx_curr,1));
                    Hess = -inv(Pp_curr) -M_curr{jj}'*(R_parconst.Value\M_curr{jj}) +term; % ; %  
                     
                    % Check positive definiteness
                    [~,p] = chol(-(Hess' * Hess) \ Hess');
                    % If not, revert to standard IEKF update
                    if p>0
                        Hess = Hess-term;
                    end
                    
                    % Update the reference point for the linearized filter estimate
                    % by treating as Newton-Raphoson optimization
                    eta_next  = eta_curr - (Hess' * Hess) \ Hess' * Jacob;
                end
                % Computing the Kalman gain, K
                % MP_term = Pp_curr *M_curr{jj}';
                % block1 = M_curr{jj} * MP_term + R_parconst.Value;
                % K_curr = MP_term / block1; 
                ...
                sse_curr(jj) = (y_k-yf_next{jj})'*(R_parconst.Value\(y_k-yf_next{jj}));
                
                % Use -inv(Hess) as filter state-error covariance
                % Need to re-evaluate Hess at eta_next
                M_tmp = m.getHx(eta_next,omega,curr_combo);
                if any(~isreal(M_tmp))
                    disp('imag');
                end
                invRresid = R_parconst.Value\(y_k - yf_next{jj});
                Hxx_tmp = m.getHxx(eta_next,omega,curr_combo);
                term = squeeze(sum(repmat(invRresid,[1 length(xp_curr) length(xp_curr)]).* Hxx_tmp,1));
                Hess = -inv(Pp_curr) -M_tmp'*(R_parconst.Value\M_tmp) +term; % ; %
                [~,p] = chol(-(Hess' * Hess) \ Hess');
                if p>0
                    Hess = Hess-term;
                end
                Pf_curr{jj} = -(Hess' * Hess) \ Hess'; 
            ...
            if min(sse_pred) < min(sse_curr)
                % If not, find max likelihood prediction
                ...
                ref_cov=[];
                good_ref=false;
                % Start with the last combo as the start an loop through to see if there is a good reference
